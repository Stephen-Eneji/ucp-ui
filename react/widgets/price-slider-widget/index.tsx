...
ReactRender(({ coins, settings }: { coins: CoinData[], settings: UCPWidgetSetting }) => {
	...
	return (
		<div className="ucp-price-slider-widget" style={{ width: parentWidth }}>
			{
				settings.card == 'card-002' && <div className={"ucp-price-slider-control"}>
					<div className="ucp-price-slider-control-btn" onClick={() => setDirection('left')}><ArrowLeft2 size="32"  variant="Outline"/></div>
					<div className="ucp-price-slider-control-btn" onClick={() => setDirection('right')}><ArrowRight2 size="32"  variant="Outline"/></div>
				</div>
			}
			<Marquee className="ucp-price-slider-widget-marquee" style={{ display:'flex', gap: '10px' }} pauseOnHover={true} speed={animationDuration} gradient={true} gradientWidth={50} direction={direction}>
				<div className={`ucp-price-slider-holder-cnt ${settings.card == 'card-002' ? 'ucp-price-slider-dark' : ''}`}>
					{
