...
const Card003 = ({coinData, currency_symbol = "$", graph_days_count = 7, max_point_graph = 15, ...props} : {coinData: CoinData, currency_symbol?: string, graph_days_count ?: number, max_point_graph ?: number} & HTMLProps<HTMLDivElement>) => {
	...
	useEffect(() => {
		...
		UCPAPIV1.fetchData<{prices : [number, number][]}>('coin-chart-data', {coin_id :coinData.id, days:graph_days_count}).then((data) => {
	...
	return (
		<div  {...props}>
			...
			<Graph  chartData={graphData} className="ucp-coin-marquee-coin-graph" labels={graphLabels} defaultDataSetSettings={defaultDataSetSettings} />
